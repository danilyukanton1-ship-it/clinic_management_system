...
class ScheduleSlotService:
    ...
    async def _create_slots(
        ...
        while slot_start < workday_end:
            ...
            if absences and any(
                slot_start < absence.end_date and slot_end > absence.start_date
        ...
        async with self.uow:
            ...
            while current_date <= data.end_date:
                ...
                slot_start = datetime.combine(
                    current_date, schedule.start_time, tzinfo=UTC
                )

                workday_end = datetime.combine(
                    current_date, schedule.end_time, tzinfo=UTC
                )
    ...
    async def change_slot_status(
        self, slot_id: int, status: SlotStatus
    ) -> ScheduleSlotResponseSchema:
        async with self.uow:
    ...
    async def update(
        ...
        async with self.uow:
            ...
            if slot.status in (SlotStatus.BOOKED, SlotStatus.BLOCKED):
                raise SlotCanNotBeChangedException()
            overlapping_slots = (
                await self.uow.schedule_slots.get_slots_overlapping_period(

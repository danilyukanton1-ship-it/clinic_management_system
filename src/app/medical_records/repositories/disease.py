...
class DiseaseRepository(BaseRepository):
    ...
    async def get_disease_by_code(self, disease_code: str) -> Disease | None:
        stmt = select(Disease).where(Disease.code == disease_code)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_disease_by_name(self, disease_name: str) -> Disease | None:
        stmt = select(Disease).where(Disease.name == disease_name)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def get_diseases_by_ids(self, disease_ids: list) -> list[Disease]:
        stmt = select(Disease).where(Disease.id.in_(disease_ids))
        result = await self.session.execute(stmt)
        return list(result.scalars().all())

    async def update_disease(
        self, disease: Disease, data: DiseaseUpdateSchema
    ) -> Disease:

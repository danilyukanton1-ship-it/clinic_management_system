...
class ScheduleAbsenceService:
    ...
    async def _unblock_slots_for_absence(
        ...
        slots_to_unblock = await self.uow.schedule_slots.get_future_blocked_slots_in_period(
            doctor_id=doctor_id,
            start_date=start_date,
            end_date=end_date,
        )
    ...
    async def update(self, absence_id: int, data: ScheduleAbsenceUpdateSchema) -> ScheduleAbsenceResponseSchema:
        async with self.uow:
            ...
            if absence.start_date <= now:
                if data.start_date.replace(microsecond=0) != absence.start_date.replace(microsecond=0):

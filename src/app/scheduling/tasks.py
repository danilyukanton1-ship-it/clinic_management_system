...
async def _cleanup_inactive_old_schedules():
    async with AsyncSessionLocal() as session:
        async with UnitOfWork(session) as uow:
            ...
            appointments = await uow.appointments.get_appointments_to_delete_by_schedule_ids(
                schedule_ids=schedule_ids
            )
            for appointment in appointments:
                await uow.appointments.delete_appointment(
            ...
            for slot in slots:
                await uow.schedule_slots.delete_slot(
                    slot=slot
                )
            for schedule in schedules:
                await uow.schedules.delete_schedule(
                    schedule=schedule
                )
